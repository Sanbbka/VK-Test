...
extension Date {
    static func beginMonth() -> Date {
        return Date().dateFor(DateForType.startOfMonth)
    }
    static func beginDay() -> Date {
        return Date().dateFor(DateForType.startOfDay)
    }
    static func subMonth() -> Date {
        return Date(timeIntervalSince1970: Date().timeIntervalSince1970 - 86400*29)
    }
    static func subDay() -> Date {
        return Date(timeIntervalSince1970: Date().timeIntervalSince1970 - 86400)
    }
    
    func subMonthN() -> Date {
        return Date(timeIntervalSince1970: self.timeIntervalSince1970 - 86400*29)
    }
}
    ...
    init?(fromString string: String, format:DateFormatType, timeZone: TimeZoneType = .local, locale: Locale = Foundation.Locale.current) {
        ...
        case .rss, .altRSS:
            if string.hasSuffix("Z") {
                ...
                string = string[index...].appending("GMT")
            }
        default:
            break
        }
